...
namespace fix_math {
	...
	public:
		...
		constexpr static fixed fromRaw(const FIXED_TYPE& val);

		explicit operator float() const;
		explicit operator double() const;
		template<class T, std::enable_if_t<std::is_integral<T>::value, int> = 0>
		explicit operator T();
		[[nodiscard]] FIXED_TYPE toRaw() const;
	...
	constexpr fix_math::fixed::fixed(const float& val) {
		this->_val = static_cast<FIXED_TYPE>(val * ONE);
	}

	constexpr fix_math::fixed::fixed(const double& val) {
		this->_val = static_cast<FIXED_TYPE>(val * ONE);
	}

	template<class T>
	constexpr fixed::fixed(const T& val) {
		this->_val = static_cast<FIXED_TYPE>(static_cast<FIXED_TYPE>(val) * ONE);
	}

	template<class T, std::enable_if_t<std::is_integral<T>::value, int>>
	fixed::operator T() {
		return static_cast<T>(this->_val / ONE);
	}

...
public:
	...
	void toBeCloseTo(T expected, double precision = 0.01);
...
template<class T>
void Matcher<T>::toBeCloseTo(T expected, double precision) {
	auto p {std::max(fabs(expected * precision), precision)};

	if (fabs(static_cast<double>(_actual - expected)) <= std::max(fabs(expected * precision), precision)) {
		return;
	} else {
		throw AssertException(!_negated ?
				"Actual value does not match the expected value" :
